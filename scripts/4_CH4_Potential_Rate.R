...
### CH4 rate calculation ###

# ---- Load VS data ----
VS<-read_excel(here("data/Slurry","VS_slurry.xlsx"))

# ---- Read AMPTS reports ----
CH4_rate <- function(Per, file) {
  file_path <- switch(
    as.character(file),
    "1" = "CH4_rate/report_2022-06-27_day.xlsx",
    "2" = "CH4_rate/report_2022-07-21_day.xlsx",
    "3" = "CH4_rate/report_2022-07-21_day.xlsx",
    "4" = "CH4_rate/report_2022-08-08_day.xlsx",
    "5" = "CH4_rate/report_2022-10-10_day.xlsx",
    "6" = "CH4_rate/report_2022-11-01_day.xlsx",
    "7" = "CH4_rate/report_2022-12-20_day.xlsx",
    "8" = "CH4_rate/report_2023-01-10_day.xlsx",
    "9" = "CH4_rate/report_2023-02-02_day_S5.2.xlsx",
    stop("Invalid data value. Please specify correct number.")
  )
  data <- read_excel(here("data", file_path), skip = 15)
...
for (i in 1:9){ #Specify the files to read
...
list_compare[[i]]<-rates_df
  ...
  list_MPR[[i]]<-rates2_df
}

MPR<-bind_rows(list_MPR)
MPR[MPR == Inf | MPR == -Inf] <- NA

Rate_S5_Max<-rowMeans(MPR[,c(2,4,6)],na.rm=TRUE)
Rate_S6_Max<-rowMeans(MPR[,c(8,10,12)],na.rm=TRUE)
